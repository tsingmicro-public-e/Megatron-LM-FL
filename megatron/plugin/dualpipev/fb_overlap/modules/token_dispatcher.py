...
def preprocess(self, routing_map: torch.Tensor) -> torch.Tensor:
    ...
    if self.drop_and_pad:
        ...
        num_tokens_per_local_expert = torch.full(
            (self.num_local_experts,), self.capacity * self.tp_size * self.ep_size, dtype=torch.long
        )
    ...
    if self.config.moe_expert_capacity_factor is not None or self.config.moe_router_padding_for_fp8:
...
def alltoall_token_perm1(
    ...
    tokens_per_expert = _maybe_dtoh_and_synchronize(self, "before_permutation_1", tokens_per_expert)
    ...
    tokens_per_expert = _maybe_dtoh_and_synchronize(self, "before_ep_alltoall", tokens_per_expert)
...
def alltoall_token_unperm1(self, hidden_states: torch.Tensor, bias: torch.Tensor = None):
