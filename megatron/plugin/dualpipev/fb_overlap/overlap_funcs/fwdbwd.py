# Mainly adopted from https://gitee.com/ascend/MindSpeed/blob/master/mindspeed/core/transformer/moe/moe_feature/fb_overlap/overlap_funcs/fwdbwd.py
...
def transformer_layer_forward_dense_backward_moe_overlapping(
    fwd_layer,
    hidden_states,
    attention_mask,
    ...
    """Forward-backward overlapping function of transformer layer, forward dense backward moe"""
    ...
    with checkpoint_context:
        ...
        attention_out, detached_attention_out = hidden_states, detach_tensor(
            hidden_states, checkpoint_forward=checkpoint
        )
    ...
    with checkpoint_context:
        with fwd_layer.bias_dropout_add_exec_handler():
            hidden_states = fwd_layer.mlp_bda(
                fwd_layer.training, fwd_layer.config.bias_dropout_fusion
    ...
    graph = LayerGraph(saved_tensors, [], None, None, fwd_layer, checkpointed=checkpoint)
...
def transformer_layer_forward_moe_backward_dense_overlapping(
    fwd_layer,
    hidden_states,
    attention_mask,
    bwd_layer_output_grad=None,
    ...
    """Forward-backward overlapping function of transformer layer, forward moe backward dense"""
    args = get_args()
    use_shared_experts = (
        hasattr(fwd_layer.mlp, 'shared_experts') and fwd_layer.mlp.shared_experts is not None
    )
    if checkpoint:
        checkpoint_context = torch.no_grad()
    else:
    ...
    with checkpoint_context:
        ...
        perm1_local_input_tokens, perm1_probs, tokens_per_expert = alltoall_token_perm1(
            fwd_layer.mlp.token_dispatcher, detached_mlp_input, probs_detached, routing_map
        )
    ...
    run_graph_backward(
        bwd_layer_graph.unperm2_graph, bwd_layer_output_grad, keep_grad=True
    )  # keep for dw
    run_graph_backward(bwd_layer_graph.pre_mlp_layernorm_graph, keep_graph=True)
    ...
    with checkpoint_context:
        detached_perm1_local_input_tokens_a2a = detach_tensor(
            perm1_local_input_tokens_a2a, checkpoint_forward=checkpoint
        )
        detached_perm1_probs_a2a = detach_tensor(perm1_probs_a2a, checkpoint_forward=checkpoint)
        dispatched_input, tokens_per_expert, permuted_probs = alltoall_token_perm2(
            fwd_layer.mlp.token_dispatcher,
            detached_perm1_local_input_tokens_a2a,
            detached_perm1_probs_a2a,
            tokens_per_expert,
        )
        ...
        expert_output, mlp_bias = fwd_layer.mlp.experts(
            detached_dispatched_input, tokens_per_expert, detached_permuted_probs
        )
        ...
        unperm1_hidden_states = alltoall_token_unperm1(
            fwd_layer.mlp.token_dispatcher, detached_expert_output, detached_mlp_bias
        )
    ...
    with checkpoint_context:
        detached_unperm1_hidden_states_a2a = detach_tensor(
            unperm1_hidden_states_a2a, checkpoint_forward=checkpoint
        )
        route_expert_output, _ = alltoall_token_unperm2(
            fwd_layer.mlp.token_dispatcher, detached_unperm1_hidden_states_a2a
        )

        if use_shared_experts:
            shared_experts_graph = shared_expert_output
            detached_shared_expert_output = detach_tensor(
                shared_expert_output, checkpoint_forward=checkpoint
            )
...
def transformer_layer_forward_dense_backward_dense_overlapping(
    fwd_layer,
    hidden_states,
    attention_mask,
    bwd_layer_output_grad=None,
    ...
    """Forward-backward overlapping function of transformer layer, forward dense backward dense"""
    if checkpoint:
        checkpoint_context = torch.no_grad()
    else:
        checkpoint_context = nullcontext()
...
def transformer_layer_forward_moe_backward_moe_overlapping(
    fwd_layer,
    hidden_states,
    attention_mask,
    bwd_layer_output_grad=None,
    ...
    """Forward-backward overlapping function of transformer layer, forward moe backward moe"""
    if checkpoint:
        checkpoint_context = torch.no_grad()
    else:
        checkpoint_context = nullcontext()
